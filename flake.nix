{
  ...
  outputs = {
    ...
    lsdir = dir:
      if (builtins.pathExists (src + "/${dir}"))
      then (lib.attrNames (lib.filterAttrs (path: type: type == "directory") (builtins.readDir (src + "/${dir}"))))
      else [];
    ...
    nixosConfigurations = let
      constructSystem = {
        ...
        lib.nixosSystem {
          ...
          modules =
            [nixos-modules.nixosModule sops-nix.nixosModules.sops {config.networking.hostName = "${hostname}";}]
            ++ (
              if server
              then [
            ...
            ++ (
              if home
              then (map (user: {home-manager.users.${user} = import ./users/${user}/home.nix;}) users)
              else []
            )
            ++ map
            (user: {
            ...
            (map
              (machine: {
                ${machine.pkgs.system} = builtins.listToAttrs (builtins.filter (v: v != {}) (map
                  (pkg: (
                    if (builtins.hasAttr pkg.name pkgsBySystem.${machine.pkgs.system})
                    then {
                      name = pkg.name;
        ...
        let
          mkBuild = type: let
            getBuildEntryPoint = name: nixosSystem:
              if builtins.hasAttr type nixosSystem.config.system.build
              then let
                cfg = nixosSystem.config.system.build.${type};
              in
                if nixosSystem.config.nixpkgs.system == "aarch64-linux"
                then lib.recursiveUpdate cfg {meta.timeout = 24 * 60 * 60;}
                else cfg
              else {};
          in
